<?php
class ControllerExtensionModuleRkExportImport extends Controller { 
	private $error = array();
	private $ssl = 'SSL';
	...
	public function index() {
		$this->load->language('extension/module/rk_export_import');
		$this->document->setTitle($this->language->get('heading_title'));
		$this->load->model('extension/module/rk_export_import');
		$this->getForm();
	}
	...
	public function upload() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateUploadForm())) {
			if ((isset( $this->request->files['upload'] )) && (is_uploaded_file($this->request->files['upload']['tmp_name']))) {
				...
				if ($this->model_extension_module_rk_export_import->upload($file,$this->request->post['incremental'])==true) {
					$this->session->data['success'] = $this->language->get('text_success');
					$this->response->redirect($this->url->link('extension/module/rk_export_import', 'user_token=' . $this->session->data['user_token'], $this->ssl));
				}
				else {
	...
	public function download() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateDownloadForm()) {
			...
			switch ($export_type) {
				...
				case 'u':
					...
					if (($min==null) || ($max==null)) {
						$this->model_extension_module_rk_export_import->download($export_type, null, null, null, null);
					} else if ($this->request->post['range_type'] == 'id') {
						$this->model_extension_module_rk_export_import->download($export_type, null, null, $min, $max);
					} else {
						$this->model_extension_module_rk_export_import->download($export_type, $min*($max-1-1), $min, null, null);
					}
					break;
				case 'o':
					$this->model_extension_module_rk_export_import->download('o', null, null, null, null);
					break;
				case 'a':
					$this->model_extension_module_rk_export_import->download('a', null, null, null, null);
					break;
				case 'f':
					if ($this->model_extension_module_rk_export_import->existFilter()) {
						$this->model_extension_module_rk_export_import->download('f', null, null, null, null);
						break;
					}
					break;
				default:
			...
			$this->response->redirect( $this->url->link( 'extension/module/rk_export_import', 'user_token='.$this->request->get['user_token'], $this->ssl) );
		}
	...
	public function settings() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validateSettingsForm())) {
			if (!isset($this->request->post['rk_export_import_settings_use_export_cache'])) {
				$this->request->post['rk_export_import_settings_use_export_cache'] = '0';
			}
			if (!isset($this->request->post['rk_export_import_settings_use_import_cache'])) {
				$this->request->post['rk_export_import_settings_use_import_cache'] = '0';
			}
			$this->load->model('setting/setting');
			$this->model_setting_setting->editSetting('rk_export_import', $this->request->post);
	...
	protected function getForm() {
		...
		$data['exist_filter'] = $this->model_extension_module_rk_export_import->existFilter();
		...
		if (!empty($this->session->data['rk_export_import_error']['errstr'])) {
			$this->error['warning'] = $this->session->data['rk_export_import_error']['errstr'];
		}

 		if (isset($this->error['warning'])) {
			$data['error_warning'] = $this->error['warning'];
			if (!empty($this->session->data['rk_export_import_nochange'])) {
		...
		unset($this->session->data['rk_export_import_error']);
		unset($this->session->data['rk_export_import_nochange']);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/module/rk_export_import', 'user_token=' . $this->session->data['user_token'], $this->ssl)
		);

		$data['back'] = $this->url->link('common/dashboard', 'user_token=' . $this->session->data['user_token'], $this->ssl);
		$data['button_back'] = $this->language->get( 'button_back' );
		$data['import'] = $this->url->link('extension/module/rk_export_import/upload', 'user_token=' . $this->session->data['user_token'], $this->ssl);
		$data['export'] = $this->url->link('extension/module/rk_export_import/download', 'user_token=' . $this->session->data['user_token'], $this->ssl);
		$data['settings'] = $this->url->link('extension/module/rk_export_import/settings', 'user_token=' . $this->session->data['user_token'], $this->ssl);
		...
		if (isset($this->request->post['rk_export_import_settings_use_option_id'])) {
			$data['settings_use_option_id'] = $this->request->post['rk_export_import_settings_use_option_id'];
		} else if ($this->config->get( 'rk_export_import_settings_use_option_id' )) {
			$data['settings_use_option_id'] = '1';
		} else {
			$data['settings_use_option_id'] = '0';
		}

		if (isset($this->request->post['rk_export_import_settings_use_option_value_id'])) {
			$data['settings_use_option_value_id'] = $this->request->post['rk_export_import_settings_use_option_value_id'];
		} else if ($this->config->get( 'rk_export_import_settings_use_option_value_id' )) {
			$data['settings_use_option_value_id'] = '1';
		} else {
			$data['settings_use_option_value_id'] = '0';
		}

		if (isset($this->request->post['rk_export_import_settings_use_attribute_group_id'])) {
			$data['settings_use_attribute_group_id'] = $this->request->post['rk_export_import_settings_use_attribute_group_id'];
		} else if ($this->config->get( 'rk_export_import_settings_use_attribute_group_id' )) {
			$data['settings_use_attribute_group_id'] = '1';
		} else {
			$data['settings_use_attribute_group_id'] = '0';
		}

		if (isset($this->request->post['rk_export_import_settings_use_attribute_id'])) {
			$data['settings_use_attribute_id'] = $this->request->post['rk_export_import_settings_use_attribute_id'];
		} else if ($this->config->get( 'rk_export_import_settings_use_attribute_id' )) {
			$data['settings_use_attribute_id'] = '1';
		} else {
			$data['settings_use_attribute_id'] = '0';
		}

		if (isset($this->request->post['rk_export_import_settings_use_filter_group_id'])) {
			$data['settings_use_filter_group_id'] = $this->request->post['rk_export_import_settings_use_filter_group_id'];
		} else if ($this->config->get( 'rk_export_import_settings_use_filter_group_id' )) {
			$data['settings_use_filter_group_id'] = '1';
		} else {
			$data['settings_use_filter_group_id'] = '0';
		}

		if (isset($this->request->post['rk_export_import_settings_use_filter_id'])) {
			$data['settings_use_filter_id'] = $this->request->post['rk_export_import_settings_use_filter_id'];
		} else if ($this->config->get( 'rk_export_import_settings_use_filter_id' )) {
		...
		if (isset($this->request->post['rk_export_import_settings_use_export_cache'])) {
			$data['settings_use_export_cache'] = $this->request->post['rk_export_import_settings_use_export_cache'];
		} else if ($this->config->get( 'rk_export_import_settings_use_export_cache' )) {
		...
		if (isset($this->request->post['rk_export_import_settings_use_import_cache'])) {
			$data['settings_use_import_cache'] = $this->request->post['rk_export_import_settings_use_import_cache'];
		} else if ($this->config->get( 'rk_export_import_settings_use_import_cache' )) {
		...
		$min_product_id = $this->model_extension_module_rk_export_import->getMinProductId();
		$max_product_id = $this->model_extension_module_rk_export_import->getMaxProductId();
		$count_product = $this->model_extension_module_rk_export_import->getCountProduct();
		$min_category_id = $this->model_extension_module_rk_export_import->getMinCategoryId();
		$max_category_id = $this->model_extension_module_rk_export_import->getMaxCategoryId();
		$count_category = $this->model_extension_module_rk_export_import->getCountCategory();
		$min_customer_id = $this->model_extension_module_rk_export_import->getMinCustomerId();
		$max_customer_id = $this->model_extension_module_rk_export_import->getMaxCustomerId();
		$count_customer = $this->model_extension_module_rk_export_import->getCountCustomer();
		...
		$this->document->addStyle('view/stylesheet/rk_export_import.css');

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view( 'extension/module/rk_export_import', $data));
	}


	protected function validateDownloadForm() {
		if (!$this->user->hasPermission('access', 'extension/module/rk_export_import')) {
			$this->error['warning'] = $this->language->get('error_permission');
			return false;
		}

		if (!$this->config->get( 'rk_export_import_settings_use_option_id' )) {
			$option_names = $this->model_extension_module_rk_export_import->getOptionNameCounts();
		...
		if (!$this->config->get( 'rk_export_import_settings_use_option_value_id' )) {
			$option_value_names = $this->model_extension_module_rk_export_import->getOptionValueNameCounts();
		...
		if (!$this->config->get( 'rk_export_import_settings_use_attribute_group_id' )) {
			$attribute_group_names = $this->model_extension_module_rk_export_import->getAttributeGroupNameCounts();
		...
		if (!$this->config->get( 'rk_export_import_settings_use_attribute_id' )) {
			$attribute_names = $this->model_extension_module_rk_export_import->getAttributeNameCounts();
		...
		if (!$this->config->get( 'rk_export_import_settings_use_filter_group_id' )) {
			$filter_group_names = $this->model_extension_module_rk_export_import->getFilterGroupNameCounts();
		...
		if (!$this->config->get( 'rk_export_import_settings_use_filter_id' )) {
			$filter_names = $this->model_extension_module_rk_export_import->getFilterNameCounts();
	...
	protected function validateUploadForm() {
		if (!$this->user->hasPermission('modify', 'extension/module/rk_export_import')) {
			$this->error['warning'] = $this->language->get('error_permission');
	...
	protected function validateSettingsForm() {
		...
		if (empty($this->request->post['rk_export_import_settings_use_option_id'])) {
			$option_names = $this->model_extension_module_rk_export_import->getOptionNameCounts();
			foreach ($option_names as $option_name) {
				if ($option_name['count'] > 1) {
					$this->error['warning'] = str_replace( '%1', $option_name['name'], $this->language->get( 'error_option_name' ) );
		...
		if (empty($this->request->post['rk_export_import_settings_use_option_value_id'])) {
			$option_value_names = $this->model_extension_module_rk_export_import->getOptionValueNameCounts();
			foreach ($option_value_names as $option_value_name) {
				if ($option_value_name['count'] > 1) {
					$this->error['warning'] = str_replace( '%1', $option_value_name['name'], $this->language->get( 'error_option_value_name' ) );
		...
		if (empty($this->request->post['rk_export_import_settings_use_attribute_group_id'])) {
			$attribute_group_names = $this->model_extension_module_rk_export_import->getAttributeGroupNameCounts();
			foreach ($attribute_group_names as $attribute_group_name) {
				if ($attribute_group_name['count'] > 1) {
					$this->error['warning'] = str_replace( '%1', $attribute_group_name['name'], $this->language->get( 'error_attribute_group_name' ) );
		...
		if (empty($this->request->post['rk_export_import_settings_use_attribute_id'])) {
			$attribute_names = $this->model_extension_module_rk_export_import->getAttributeNameCounts();
			foreach ($attribute_names as $attribute_name) {
				if ($attribute_name['count'] > 1) {
					$this->error['warning'] = str_replace( '%1', $attribute_name['name'], $this->language->get( 'error_attribute_name' ) );
		...
		if (empty($this->request->post['rk_export_import_settings_use_filter_group_id'])) {
			$filter_group_names = $this->model_extension_module_rk_export_import->getFilterGroupNameCounts();
			foreach ($filter_group_names as $filter_group_name) {
				if ($filter_group_name['count'] > 1) {
					$this->error['warning'] = str_replace( '%1', $filter_group_name['name'], $this->language->get( 'error_filter_group_name' ) );
		...
		if (empty($this->request->post['rk_export_import_settings_use_filter_id'])) {
			$filter_names = $this->model_extension_module_rk_export_import->getFilterNameCounts();
			foreach ($filter_names as $filter_name) {
				if ($filter_name['count'] > 1) {
					$this->error['warning'] = str_replace( '%1', $filter_name['name'], $this->language->get( 'error_filter_name' ) );
	...
	public function getNotifications() {
		...
		$this->load->model('extension/module/rk_export_import');
		$this->load->language( 'extension/module/rk_export_import' );
		$response = $this->model_extension_module_rk_export_import->getNotifications();
		$json = array();
		if ($response===false) {
			$json['message'] = '';
	...
	public function getCountProduct() {
		$this->load->model('extension/module/rk_export_import');
		$count = $this->model_extension_module_rk_export_import->getCountProduct();
